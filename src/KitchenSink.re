...
let setup = (assetDir, env) => {
  Env.size(~width=400, ~height=500, env);
  {
    ...
    font:
      Draw.loadFont(
        ~filename=Filename.concat(assetDir, "SFCompactDisplay-Regular-16.fnt"),
        ~isPixel=false,
        env
      )
  }
...
let matrix = (env, t) => {
  ...
  Draw.curve(((-500.), 1000.), ((-100.), 50.), (100., 50.), (500., (-1000.)), env);
  Draw.stroke(Utils.color(~r=0, ~g=200, ~b=100, ~a=200), env);
  Draw.line(~p1=((-100), (-50)), ~p2=(100, (-50)), env);
  Draw.fill(Utils.color(~r=255, ~g=200, ~b=255, ~a=200), env);
  Draw.ellipse(~center=((-100), (-100)), ~radx=20, ~rady=20, env);
  Draw.quad(~p1=(100, (-100)), ~p2=(150, (-150)), ~p3=(100, (-150)), ~p4=(150, (-100)), env);
  Draw.pixel(~pos=(50, 100), ~color=Constants.white, env);
  Draw.triangle(~p1=(0, (-100)), ~p2=(50, (-150)), ~p3=(0, (-150)), env);
  Draw.bezier(
    ~p1=((-50.), 100.),
    ~p2=((-100.), 150.),
    ~p3=(150., 150.),
